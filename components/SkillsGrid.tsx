    ...
    <section id="skills" className="py-20 px-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {SKILL_CATEGORIES.map((cat, idx) => (
          <div key={idx} className="bento-card p-6 rounded-3xl flex flex-col">
            <div className="mb-4 p-3 bg-zinc-800/50 rounded-2xl w-fit">
            ...
            <div className="flex flex-wrap gap-2 mt-auto">
              {cat.skills.map(skill => (
                <span key={skill} className="px-3 py-1 bg-zinc-900/50 border border-zinc-800 text-zinc-400 rounded-full text-[10px] hover:border-zinc-600 transition-colors uppercase tracking-wider font-medium">
                  {skill}
                </span>
              ))}
            </div>
